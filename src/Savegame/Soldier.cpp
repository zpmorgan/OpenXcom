...
/** Initializes a soldier from a BattleUnit, for field recruitment/defections
 * @param rules Unused but included to set the object's _rules to a valid value out of paranoia
 * @param bu the Data source
 * @param name A name
 * @param id A new id generated by _game->getSavedGame()->getId("STR_SOLDIER")
 */
Soldier::Soldier(RuleSoldier *rules, BattleUnit *bu, std::wstring name, int id): _name(name), _id(id), _improvement(0), _rules(rules), _initialStats(), _currentStats(), _rank(RANK_ROOKIE), _craft(0), _gender(GENDER_MALE), _look(LOOK_BLONDE), _missions(0), _kills(0), _recovery(0), _recentlyPromoted(false), _psiTraining(false), _equipmentLayout()
{
	_initialStats = _currentStats = *(bu->getStats());
	_armor = bu->getArmor();
	_gender = bu->getGender();
	_type = bu->getType();
}
...
void Soldier::load(const YAML::Node &node, const Ruleset *rule)
{
	...
	{
		// value is optional to retain backward compatibility with vanilla OXC saves
	}
